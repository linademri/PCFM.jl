...
Compile the FNO model for faster execution. Requires `using Reactant` to be

loaded in the session — without it, raises an informative error.



Returns the compiled model function.
...
function compile_model(ffm::FFM)
    ...
    # Reactant compilation is provided by PCFMReactantExt when Reactant is loaded.

    # Without the extension, this errors with a helpful message.

    model_compiled = _reactant_compile(() -> ffm.model(x_test, ffm.ps, Lux.testmode(ffm.st)))
...
Compile all helper functions (model, interpolation, input preparation).

Requires `using Reactant` — errors otherwise via the `_reactant_compile` stub.
...
function compile_functions(ffm::FFM, batch_size::Int)
    ...
    # Compile via the extension stub. Errors if Reactant isn't loaded.

    model_compiled = _reactant_compile(() -> ffm.model(x_test, ffm.ps, Lux.testmode(ffm.st)))



    interpolate_compiled = _reactant_compile(

        () -> interpolate_flow(t_test, x_0_test, data_test, batch_size)
    ...
    prepare_input_compiled = _reactant_compile(

        () -> prepare_input(x_t_test, t_test, nx, nt, batch_size, emb_channels)
